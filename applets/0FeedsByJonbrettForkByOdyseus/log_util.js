...
function Logger(aOptions) {
    this.uuid = aOptions.uuid || "";
    this.verbose = aOptions.verbose || false;
}

Logger.prototype.debug = function(aMsg) {
    // Only log when verbose logging is enabled
    if (this.verbose) {
        global.log(this.uuid + " :: " + aMsg);
    }
};

Logger.prototype.info = function(aMsg) {
    // always display info logging
    global.log(this.uuid + " :: " + aMsg);
};

Logger.prototype.error = function(aMsg) {
    // always display error logging
    global.logError(this.uuid + " :: ERROR :: " + aMsg);
};
